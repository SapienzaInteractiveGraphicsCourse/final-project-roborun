...
function createScene() {
  ...
  //start game image
  var startGame = new BABYLON.GUI.Image("", "img/startGame.png");
  if (document.body.clientWidth > document.body.clientHeight) {
    startGame.width = 0.4;
  ...
  sidewalkMat.diffuseTexture = new BABYLON.Texture("textures/grass.jpg", scene);
  ...
  function robotMoveLeft() {
    if (robotHead.position.x == 0 || robotHead.position.x == 3) {
      moveRobotAnim(robotHead, robotHead.position.x, robotHead.position.x - 3);
  ...
  startGame.onPointerClickObservable.add(function () {
    const cameraAnimations = [animMove(camera, new BABYLON.Vector3(0, 3, -5))];
    startGame.notRenderable = true;
    setTimeout(function () {
